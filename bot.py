...
# ╔══════════════════════════════════════════════════════════════════╗
# ║              📝 সহজ CONFIGURATION GUIDE — এখানে লিখবেন          ║
# ╠══════════════════════════════════════════════════════════════════╣
# ║  মনে রাখবেন:                                                      ║
# ║  OLD / SEARCH TAG  = পুরোনো পোস্ট খুঁজতে যে tag ব্যবহার হবে       ║
# ║  NEW / SOURCE TAG  = Source-এ নতুন refreshed copy-তে লেখা হবে     ║
# ║  TARGET TAG       = Target channel-এর নতুন পোস্টে লেখা হবে       ║
# ╚══════════════════════════════════════════════════════════════════╝

# ── ১) Source channel শনাক্তকরণ ───────────────────────────────────
# Source channel-এর username এখানে লিখুন (@ ছাড়া)।
# Username না থাকলে "" লিখে শুধু CHANNEL_ID ব্যবহার করা যাবে।
CHANNEL_USERNAME = "ALQalamBD"
CHANNEL_ID       = -1003797236998       # Username কাজ না করলে backup ID

# ── ২) Target channel — নতুন copy এখানে পোস্ট হবে ─────────────────
# Target channel-এর কোনো পুরোনো message এই code delete করবে না।
TARGET_CHANNEL_ID       = -1003704917412
TARGET_CHANNEL_USERNAME = "Islamic_Stor" # @ ছাড়া; resolve না হলে ID ব্যবহার হবে

# ── ৩) Source-এর পুরোনো post খোঁজার tag ────────────────────────────
# পুরোনো Source post-এ বর্তমানে যে tag আছে, সেটি এখানে লিখবেন।
# এই tag শুধু POST খোঁজার জন্য; নতুন post-এ এই tag আর লেখা হবে না।
SOURCE_OLD_CAPTION_TAGS        = "#ALQalamBD"
FRIDAY_SOURCE_OLD_CAPTION_TAGS = "#ALQalamBD"

# ── ৪) Source channel-এ নতুন refreshed copy-তে লেখা tag ────────────
# Source-এর পুরোনো post copy হওয়ার পর caption-এর শেষে এগুলো লেখা হবে।
# একাধিক tag হলে comma বা space দিয়ে লিখুন।
SOURCE_CAPTION_TAGS        = "#ALQalam365"
FRIDAY_SOURCE_CAPTION_TAGS = "#ALQalam365, #Jumuah_Mubarak"

# ── ৫) Target channel-এ নতুন copy-তে লেখা tag ──────────────────────
TARGET_CAPTION_TAGS        = "#IslamicStore"
FRIDAY_TARGET_CAPTION_TAGS = "#Jumuah_Mubarak"

# ── ৬) Target channel join request approval ────────────────────────
# প্রতি scheduled run-এ সর্বোচ্চ কতটি pending request approve করবে।
# ০ দিলে approval বন্ধ থাকবে।
...
def has_source_tag(caption: str) -> bool:
    c = (caption or "").lower()
    source_markers = {
        *(tag.lower() for tag in parse_tags(SOURCE_OLD_CAPTION_TAGS)),
        *(tag.lower() for tag in parse_tags(FRIDAY_SOURCE_OLD_CAPTION_TAGS)),
